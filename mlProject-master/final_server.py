...
def handler():
    ...
    #read LL value from LL.txt generated by server
    ...
    '''
    #read the name of the .mat file uploaded in server
    with open('./name.txt','r') as f:
        ni=f.readlines()
    xs = ni[0]
    '''
    # load the .mat Matlab file
    x1 = loadmat('./uploads/1.mat')
    x = x1['data']
    ...
    # axs[0].plot(t, x_ecg) #we can plot(xx1[:3000])
    # axs[0].grid(True)
    # axs[1].plot(t, x_scg) #wse can plot(xx2[:3000])
    # fig.tight_layout()
    # axs[1].grid(True)
    # plt.show()
    ...
    c = []
    def returnval(x,y): #returns value of x , y being indexes
        for i in y:
            c.append(x[i])
        return c
    ...
    col = ['blue','green','red','cyan','magenta','yellow','black','white','0.75','0.95','0.82','0.71']
